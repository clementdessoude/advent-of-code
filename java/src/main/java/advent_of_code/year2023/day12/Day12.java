...
public class Day12 {
    ...
    public Long part1(List<String> lines) {
        return lines.stream().mapToLong(Day12::numberOfArrangements).sum();
    }

    private static long numberOfArrangements(String row) {
        String[] split = row.split(" ");
        var pattern = split[0];
        var sources = Arrays.stream(split[1].split(",")).map(Integer::parseInt).toList();

        return numberOfArrangements(pattern, sources);
    }

    record Cache(String pattern, List<Integer> brokenSources) {}

    private static final Map<Cache, Long> cache = new HashMap<>();

    private static long numberOfArrangements(String pattern, List<Integer> brokenSources) {
        ...
        if (
            pattern.length() <
            brokenSources.stream().mapToInt(i -> i).sum() + brokenSources.size() - 1
        ) {
        ...
        if (
            idxOfUnknownSource == -1 ||
            (idxOfBrokenSource < idxOfUnknownSource && idxOfBrokenSource != -1)
        ) {
            long result = considerNextAsBrokenSource(pattern, brokenSources, idxOfBrokenSource);
        ...
        Cache key1 = new Cache(pattern.substring(idxOfUnknownSource + 1), brokenSources);

        var optionIfNotBrokenSource = cache.containsKey(key1)
            ? cache.get(key1)
            : numberOfArrangements(pattern.substring(idxOfUnknownSource + 1), brokenSources);
    ...
    private static long considerNextAsBrokenSource(
        ...
        if (idxOfBrokenSource + firstBrokenSourceExpectedCount > pattern.length()) {
            return 0;
        }
        ...
        List<Integer> brokenSources1 = brokenSources.subList(1, brokenSources.size());
        long result = numberOfArrangements(substring, brokenSources1);
        cache.put(new Cache(substring, brokenSources1), result);
        return result;
    }

    public Long part2(List<String> lines) {
        var unfolds = lines.stream().map(Day12::unfold).toList();
