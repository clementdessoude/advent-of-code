...
public class Day16 {
    ...
    public Long part1(List<String> lines) {
        return new Grid(lines).energizedCount(new Position(0, -1), BeamDirection.RIGHT);
    }
    ...
    public Long part2(List<String> lines) {
        ...
        long maxRow = IntStream.range(0, lines.size())
            .mapToLong(i -> {
                Long one = new Grid(lines).energizedCount(new Position(i, -1), BeamDirection.RIGHT);
        ...
        long maxColumn = IntStream.range(0, columnCount)
            .mapToLong(j -> {
                Long one = new Grid(lines).energizedCount(new Position(-1, j), BeamDirection.DOWN);
